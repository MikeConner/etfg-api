...
class V2::AnalyticsController < ApplicationController
  ...
  # /v2/analytics?date=20180509&output=[csv|json]
  # /v2/analytics?start_date=20180509&end_date=20180520
  def index
  ...
  # /v2/analytics/:fund?date=20180509&output=[csv|json]
  # /v2/analytics/:fund?start_date=20180509&end_date=20180512
  def show
    unless params.has_key?(:date) or (params.has_key?(:start_date) and params.has_key?(:end_date))
      render :json => {:error => I18n.t('date_required')}, :status => :bad_request and return
    end
    ...
    result = []
    AnalyticV2.where(Utilities.date_clause(params, 'analytics')).where(:composite_ticker => fund).find_in_batches do |batch|
      result += AnalyticV2Serializer.extract(batch)      
    end
  ...
  # /v2/analytics/:fund/aggregate?date=20180509&group=asset_class&function=max
  # /v2/analytics/:fund/aggregate?start_date=20180509&end_date=20180512&group=asset_class&function=max
  def aggregate
    unless params.has_key?(:date) or (params.has_key?(:start_date) and params.has_key?(:end_date))
      render :json => {:error => I18n.t('date_required')}, :status => :bad_request and return
    end
    ...
    params[:group].downcase.split(/:/).each do |g|
      ...
      if AnalyticV2::VALID_GROUPS.include?(sp)
        groups.push(sp)
      else        
        render :json => {:error => I18n.t('valid_group', :groups => AnalyticV2::VALID_GROUPS.join(','))}, :status => :bad_request and return
      end
    end
    ...
    field_list = AnalyticV2.new.attributes
    ['id', 'run_date', 'as_of_date', 'composite_ticker', 'quant_grade', 'created_at', 'updated_at'].each do |f|
      field_list.delete(f)
    end
    select_clause = ""
  ...
  # /v2/analytics/products?date=20180509
  # /v2/analytics/products?start_date=20180509&end_date=20180512
  def products
    unless params.has_key?(:date) or (params.has_key?(:start_date) and params.has_key?(:end_date))
      render :json => {:error => I18n.t('date_required')}, :status => :bad_request and return
    end
    ...
    render :json => AnalyticV2.where(Utilities.date_clause(params, 'analytics')).order(:composite_ticker).map(&:composite_ticker).uniq
